...
#if defined(_MSC_VER) && !defined(IMGUI_NOGDI)
...
static void ImGui_ImplWin32_ShowWindow(ImGuiViewport* viewport)
{
    ...
    if (viewport->Flags & ImGuiViewportFlags_NoFocusOnAppearing)
        ::ShowWindowAsync(data->Hwnd, SW_SHOWNA);
    else
        ::ShowWindowAsync(data->Hwnd, SW_SHOW);
}
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport)
{
    ...
    if (data->DwStyle != new_style || data->DwExStyle != new_ex_style)
    {
        ...
        ::ShowWindowAsync(data->Hwnd, SW_SHOWNA); // This is necessary when we alter the style
